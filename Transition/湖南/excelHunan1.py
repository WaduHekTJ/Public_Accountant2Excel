...
def dropErrorData(dataframe):
    liststr = ['一、','二、','三、','四、','五、','六、','七、','八、','九、']
    listindex = []
    for i in liststr:
        for j in range(dataframe.shape[0]):
            if bool(re.search(i,str(dataframe.iloc[j,0]))):
                ...
                continue
    print(listindex)
    dataframe = dataframe.drop(listindex).reset_index()
    return dataframe
    
def exceltoDataFrame(filename : str):
    data = pd.read_excel(filename,sheet_name='年检合格人员名单(第二批)',header=None,skiprows=[0,1,2,3,4])
    data = dropErrorData(data).drop('index',axis=1)
    listIndex = []
    for i in range(data.shape[0]):
        if(data.iloc[i,0] is np.nan):
            listIndex.append(i)
        else:
            continue
...
def dataFrametoList(framename,numberOfIndex : list):
    ...
    for i in range(len(numberOfIndex)):
        if ((i % 2 == 0) & (i != len(numberOfIndex)-1)) :
            splitData.append(framename.iloc[numberOfIndex[i]+1:numberOfIndex[i+2],0:6])
        else:
            continue
    return splitData
...
def findName(splitOnce,index):
    ...
    for i in range(len(splitOnce)):
            ...
            sname = pd.DataFrame({splitOnce[i].iloc[0 if index[i]==4 else 1,0]})
...
if __name__ == '__main__':
    file = '湖南省注册会计师2018年度任职资格检查合格人员名单（第二批）.xlsx'
