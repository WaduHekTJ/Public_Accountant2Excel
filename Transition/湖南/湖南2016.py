...
def getTitle(paragraph:list):
    title = []  #事务所名称
    index = []  #事务所信息在paragraph中的相对位置
    for p,i in enumerate(paragraph): #除去空行
        if '事务所' in i and '注册会计师' not in i:
            title.append(i)
            index.append(p) 
        else:
            continue
...
def cleanFooter(single_shiwusuo):
    
    drop_number = 0 # 事务所中无关信息的删除次数
    for i in range(len(single_shiwusuo)):
        if re.search('第[0-9]+页', single_shiwusuo[i].replace(' ','')):
            single_shiwusuo[i] = '废弃'
            drop_number = drop_number + 1
        else:
            continue
        
    for i in range(drop_number):
        single_shiwusuo.remove('废弃')
        
    return single_shiwusuo
def getShiwusuo(paragraph:list,index:list):
    ...
    while i < len(index):
        if i == len(index)-1:
            temp_shiwusuo = paragraph[index[i]:len(paragraph)]
            shiwusuo.append(cleanFooter(temp_shiwusuo))
        else:
            temp_shiwusuo = paragraph[index[i]:index[i+1]]
            shiwusuo.append(cleanFooter(temp_shiwusuo))
        i = i + 1
...
def getPart1(shiwusuo:list):
    ...
    for i,sws in enumerate(shiwusuo):
        ...
        if len(sws) == 2:
            ...
            if ('市：' in sws[2] or '张家界：' in sws[2] or '州：' in sws[2]) and '注册会计师' in sws[2]: #如果第三行是城市信息
                name = ""
                temp_sws = sws[0].split('\n')
                #获得事务所名称
                name = temp_sws[0]
...
def findDropInformation(part1:list,table:list):
    ...
    for i,information in enumerate(part1): #识别缺失事务所信息的索引号
    ...
    for i in range(len(drop_table)):
        if len(drop_table[i][0]) == 6:
            if '主任会计师' in drop_table[i][0][2] or '合伙人' in drop_table[i][0][2] or '负责人' in drop_table[i][0][2]:
                part1[drop_index[i]][3][0] = drop_table[i][0][3].replace(' ','')
                part1[drop_index[i]].remove('缺失')
            else:
                print('出现异常'+str(i))
        
        elif len(drop_table[i][0]) == 5:
            if '主任会计师' in drop_table[i][0][1] or '合伙人' in drop_table[i][0][1] or '负责人' in drop_table[i][0][1]:
                part1[drop_index[i]][3][0] = drop_table[i][0][2].replace(' ','')
                part1[drop_index[i]].remove('缺失')
            else:
                print('出现异常'+str(i))
        else :
            print('缺失信息中出现问题'+str(i))
...
def word_extraction_Hunan_table(province:str,year:str):
    ...
    path = r'E:\Spyderworkplace\excel\湖南'
    path_output = r'E:\Spyderworkplace\excel\湖南'
...
if __name__ == '__main__':
    ...
    path = '湖南省2016年度注册会计师任职资格检查合格人员名单.docx'
    ...
    #word_extraction_Hunan_table('湖南','2016')
