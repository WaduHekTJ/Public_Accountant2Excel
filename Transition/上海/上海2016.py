...
def getTableValue(path:str):#读取各事务所中会计师姓名
    ...
    for u,t in enumerate(table):
        ...
        for v,r in enumerate(row):
            ...
            while i < len(cell):
                if(i != len(cell)-1):
                    if(cell[i].text == cell[i+1].text):
                        shiwusuo_name_rows.append(cell[i].text)
                        i = i + 2
                        continue
                    elif(len(cell[i].text) == len(cell[i+1].text) and len(cell[i].text) == 1):
                        shiwusuo_name_rows.append(cell[i].text + cell[i+1].text)
                        i = i + 2
                        continue
                    else:
                        shiwusuo_name_rows.append(cell[i].text)
...
if __name__ == "__main__":
    word_extraction_shanghai_table('上海', '2016')
